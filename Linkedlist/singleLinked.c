...
void insert_begin()
{
	...
	temp = (struct node*)malloc(sizeof(struct node));
	printf("Enter node data: ");
	scanf("%d",&temp->data); // Store the data in newly created stand alone node
	temp->next = NULL;  // Let the next pointer in newly created stand alone node intialized to NULL because normally it is going to be the last node
	if(head==NULL) // If there are no linked list node present
	{
		head = temp; // Just created new node it will represent head node so storing temp node address inside it
		return;
	}
	else
	{
		//point it to old head node, means just newly created stand along node in temp will point to our first node using head pointer which is alreadt containing
		// first node address
		temp->next = head; 
		// Let the newly created stand alone temp node now represents our first node
...
void insert_pos()
{
	struct node* temp;
        // creating a new node
	temp = (struct node*)malloc(sizeof(struct node)); // Create stand along node
	printf("Enter node data: ");
	scanf("%d",&temp->data); // Store data in the standlone node
	temp->next = NULL; // Assume that at the start standalone node next node is the last node pointing to NULL or at least it is not WILD pointer even if it is not a last node
